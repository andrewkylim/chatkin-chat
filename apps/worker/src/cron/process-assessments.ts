...
/**
 * Process a single user's assessment onboarding
 * Can be called from both cron and API endpoint
 */
export async function processSingleAssessment(userId: string, env: Env): Promise<void> {
...
export async function processUnprocessedAssessments(env: Env): Promise<void> {
	...
	for (const assessment of unprocessed) {
		await processSingleAssessment(assessment.user_id, env);
	}
