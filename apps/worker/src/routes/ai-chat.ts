...
export async function handleAIChat(
  ...
  try {
    ...
    logger.debug('Processing AI chat request', {
      ...
      workspaceContextLength: workspaceContext?.length || 0,
      workspaceContextHasFIRSTSESSION: workspaceContext?.includes('FIRST SESSION') || false,
      workspaceContextPreview: workspaceContext?.substring(0, 500) || ''
    });
